...
class ShopEmbedding(nn.Module):
    def __init__(self, vocab: Vocab, d_model: int):
        ...
        self.type_emb = nn.Embedding(5, d_model)  # card, joker, consumable, voucher, booster
    ...
    def forward(self, tokens: torch.Tensor) -> torch.Tensor:
        ...
        itype = tokens[:, :, 1].clamp(0, 4)
        price = tokens[:, :, 2].float().unsqueeze(-1)
        ed = tokens[:, :, 3].clamp(0, self.edition_emb.num_embeddings - 1)
        slot = tokens[:, :, 4].clamp(0, 9)
        return self.item_emb(iid) + self.type_emb(itype) + self.price_proj(price) + self.edition_emb(ed) + self.slot_emb(slot)
