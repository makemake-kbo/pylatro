...
def contextual_tarot_fix_reward(
    ...
    plan_evidence = max(0.0, min((pre_reliability - 0.25) / 0.5, 1.0))
    evidence = play_evidence * plan_evidence
    delta = post_quality - pre_quality
    if abs(delta) <= 0.002:
        return 0.0, pre_quality, post_quality
    ...
    if target_suit:
        if delta > 0.0 and target_suit != suit:
            return 0.0, pre_quality, post_quality
    ...
    if delta > 0.0:
        ...
        if _protected_value(post_cards) + 1e-9 < _protected_value(pre_cards):
            return 0.0, pre_quality, post_quality
        return min(4.0 * evidence * delta, 0.20), pre_quality, post_quality
    return -min(4.0 * evidence * (-delta), 0.20), pre_quality, post_quality
...
def derive_strategic_event(
    ...
    cash_payout = max(int(getattr(use_result, "dollars_delta", 0) or 0), 0)
    ...
    fix_reward, pre_quality, post_quality = contextual_tarot_fix_reward(
        prev_info,
        curr_info,
